...
class ConservativeRiskManager(BaseRiskManager):
    ...
    def active_trades(self, value):
        """Allow controlled updates from shared execution cleanup paths."""
        self.risk_manager.active_trades = value

    @property
    def has_active_trade(self) -> bool:
        """True when any conservative trade is currently active."""
        return len(self.risk_manager.active_trades) > 0
    ...
    def should_close_trade(
        ...
        """Delegate conservative exit checks to wrapped risk manager."""
        return self.risk_manager.should_close_trade(
            contract_id,
            current_pnl,
            current_price,
            previous_price,
