...
class ScalpingRiskManager(BaseRiskManager):
    ...
    Features:
    - 2 concurrent trades max (portfolio-wide)
    - 1 concurrent trade per symbol
    ...
    def can_trade(self, symbol: str = None, verbose: bool = False) -> Tuple[bool, str]:
        ...
        # CHECK 1b: Per-symbol concurrent limit (if symbol context provided)
        if symbol:
            active_for_symbol = 0
            ...
            if active_for_symbol >= self.max_concurrent_per_symbol:
                return (
                    False,
                    f"{symbol}: max concurrent trades reached ({active_for_symbol}/{self.max_concurrent_per_symbol})",
                )
