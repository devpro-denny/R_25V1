...
@pytest.mark.asyncio
async def test_analyze_symbol_manual_mode_skips_execution_and_emits_signal_metadata(runner, monkeypatch):
    ev = MagicMock()
    ev.broadcast = AsyncMock()
    monkeypatch.setattr("app.bot.runner.event_manager", ev)
    ...
    runner.user_stake = 1.0
    runner.auto_execute_signals = False
    runner.asset_config = {"R_25": {"multiplier": 10}}
    ...
    runner.strategy.analyze.return_value = {
        ...
        "take_profit": 101.0,
        "stop_loss": 99.0,
        "details": {"passed_checks": ["Trend"]},
    }
    runner.risk_manager = MagicMock()
    runner.risk_manager.can_open_trade.return_value = (True, "OK")
    runner.trade_engine = MagicMock()
    runner.trade_engine.execute_trade = AsyncMock(return_value={"contract_id": "c1"})

    assert await runner._analyze_symbol("R_25") is False
    runner.risk_manager.can_open_trade.assert_not_called()
    runner.trade_engine.execute_trade.assert_not_called()
    runner.telegram_bridge.notify_signal.assert_awaited_once()

    payloads = [c.args[0] for c in ev.broadcast.await_args_list if c.args]
    signal_events = [p for p in payloads if p.get("type") == "signal"]
    assert signal_events
    assert signal_events[-1]["execution_mode"] == "manual"
    assert signal_events[-1]["action_taken"] == "Awaiting Manual Entry"
